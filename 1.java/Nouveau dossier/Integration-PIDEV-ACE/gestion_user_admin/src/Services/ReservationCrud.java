...
public void createReservation(Reservation r) throws SQLException {
    String sql = "INSERT INTO Reservation (id_user, id_event, qte) VALUES (?, ?, ?)";
    PreparedStatement statement = connection.prepareStatement(sql);
    statement.setInt(1, r.getIdUser());
    statement.setInt(2, r.getIdEvent());
...
//Affichage normale
    ...
    //Affichage avec le tri des id_res
    public void readReservations2() throws SQLException {
    String sql = "SELECT * FROM reservation ORDER BY id_res";
    Statement statement = connection.createStatement();
    ResultSet result = statement.executeQuery(sql);
    while (result.next()) {
        int id = result.getInt("id_res");
     ...
     * @throws SQLException
     */
    public void updateReservation(int id_res, int id_user, int id_event) throws SQLException {
        String sql = "UPDATE reservation SET id_user=?, id_event=? WHERE id_res=?";
        PreparedStatement statement = connection.prepareStatement(sql);
        statement.setInt(1, id_user);
        statement.setInt(2, id_event);
        statement.setInt(3, id_res);
        statement.executeUpdate();
    }

    public void deleteReservation(int id_res) throws SQLException {
        String sql = "DELETE FROM reservation WHERE id_res=?";
        PreparedStatement statement = connection.prepareStatement(sql);
        statement.setInt(1, id_res);
        statement.executeUpdate();
    }
