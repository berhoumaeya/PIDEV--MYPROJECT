...
public class ServiceMessage implements InterfaceMessage <Message>{
    ...
    public void updateNote(Message m) throws SQLException {
        String req="update message set note=? where id_message=? AND id_post=?";
            PreparedStatement ps=con.prepareStatement(req);
            BigDecimal value = new BigDecimal(m.getNote()+"");
            ps.setBigDecimal(1, value);
            ps.setInt(2,m.getMsgId());
            ps.setInt(3,m.getPostId());   
            ps.executeUpdate();
            System.out.println(m);
        String message = "Le commentaire a été noté.";
        notifyUser(message);
    }
    
    @Override
      public void DeleteMsg(int idM, int idP) throws SQLException
     { 
        String req = "DELETE FROM message WHERE id_post=? AND id_message=?";
