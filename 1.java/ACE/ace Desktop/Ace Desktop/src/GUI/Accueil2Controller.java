...
public class Accueil2Controller implements Initializable {
    ...
    private void btnsupr(ActionEvent event) {try {
            ...
            Logger.getLogger(ResSuppController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void btnmodr(ActionEvent event) {try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("ResUpdate1.fxml"));
            Parent root = loader.load();
            btnR4.getScene().setRoot(root);

        } catch (IOException ex) {
            Logger.getLogger(ResUpdateController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
